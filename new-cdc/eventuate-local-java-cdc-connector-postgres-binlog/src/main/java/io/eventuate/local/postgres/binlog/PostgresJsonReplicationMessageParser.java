...
public class PostgresJsonReplicationMessageParser implements PostgresReplicationMessageParser<PublishedEvent> {

  @Override
  public List<PublishedEvent> parse(String message, long lastSequenceNumber) {

    try {
      PostgresReplicationMessage msg = new ObjectMapper().readValue(message, PostgresReplicationMessage.class);

      List<PostgresReplicationChange> changes = Arrays.asList(msg.getChange());

      List<PostgresReplicationChange> insertedEvents = changes
              .stream()
